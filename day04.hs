...
mostSleepingMinute :: [SleepingGuard] -> GuardId -> Int
mostSleepingMinute sgs guard = fst $ last $ sortOn snd $ freq sleeps
  where
...
solve s = do
  ...
  let guard = mostSleepingGuard guards
  let minute = mostSleepingMinute guards guard
  let (GuardId val) = guard
  return $ val * minute
...
tests = do
  ...
  describe "guard's most sleeping minute" $
    ...
    24
  describe "solve" $
    it "works with example" $ solve example `shouldBe` Right 240
