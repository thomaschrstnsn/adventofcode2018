...
overlappingSquares :: [Claim] -> Set Square
overlappingSquares claims = Set.unions $ map overlapsWithEach squares
  where
    squares = map squaresFromClaim claims
    overlapsWithEach c = Set.unions $ map (overlaps c) squares
    overlaps c1 c2 =
      if c1 == c2
        then Set.empty
...
solve s = Set.size . overlappingSquares <$> readClaims s
...
tests = do
  ...
  describe "solve" $ it "works with example" $ solve example `shouldBe` Right 4

main :: IO ()
main = do
  inp <- input
  let claims = (\c -> (head c, last c)) <$> readClaims inp
  print claims
  hspec tests
  print $ solve inp
