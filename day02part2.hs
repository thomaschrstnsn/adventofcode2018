...
arePrototypes x y = helper x y True
  where
    ...
    helper (x:xs) (y:ys) True =
      if x == y
        then helper xs ys True
        else helper xs ys False
    helper (x:xs) (y:ys) False = (x == y) && helper xs ys False
...
sameParts x y = helper x y []
  where
    ...
    helper (x:xs) (y:ys) r =
      if x == y
        then helper xs ys (r ++ [x])
        else helper xs ys r

solve :: [String] -> [String]
solve xs =
  map
    (\(x, y) -> "(" ++ show x ++ ", " ++ show y ++ "): " ++ show (sameParts x y)) $
  prototypes xs

examples = ["abcde", "fghij", "klmno", "pqrst", "fguij", "axcye", "wvxyz"]
...
tests = do
  ...
  describe "solve" $
    it "works" $ solve examples `shouldBe` ["(\"fghij\", \"fguij\"): \"fgij\""]
